...
read -p "WARNING: This will overwrite production database with local database. Are you sure you want to sync only the database from local to production? (y/N) " confirm
if [[ ! "$confirm" =~ ^[yY]$ ]]; then
  echo "Sync cancelled."
  exit 1
fi
...
echo "Sync database from local to production complete!"
