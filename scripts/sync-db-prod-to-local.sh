...
read -p "WARNING: This will overwrite local database with production database. Are you sure you want to sync only the database from production to local? (y/N) " confirm
if [[ ! "$confirm" =~ ^[yY]$ ]]; then
  echo "Sync cancelled."
  exit 1
fi
...
# Sync database
echo "Transferring and importing production database to local..."
...
echo "Sync database from production to local complete!"
