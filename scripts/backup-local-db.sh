...
BACKUP_DIR="$PARENT_DIR/backups/local/db"
TIMESTAMP=$(date +"%Y-%m-%d-%H-%M-%S")
BACKUP_FILE="$BACKUP_DIR/$LOCAL_DB_NAME-$TIMESTAMP.sql.gz"
...
docker exec $LOCAL_DB_CONTAINER mysqldump -uroot -p"$LOCAL_DB_ROOT_PASSWORD" $LOCAL_DB_NAME | gzip > $BACKUP_FILE
