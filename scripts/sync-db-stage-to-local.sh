...
read -p "WARNING: This will overwrite local database with staging database. Are you sure you want to sync only the database from stage to local? (y/N) " confirm
if [[ ! "$confirm" =~ ^[yY]$ ]]; then
  echo "Sync cancelled."
  exit 1
fi
...
: ${LOCAL_DB_ROOT_PASSWORD:? "LOCAL_DB_ROOT_PASSWORD is not set in .env"}
: ${LOCAL_DB_USER:? "LOCAL_DB_USER is not set in .env"}
: ${LOCAL_DB_PASSWORD:? "LOCAL_DB_PASSWORD is not set in .env"}
...
# Sync database
echo "Transferring and importing staging database to local..."
...
echo "Sync database from stage to local complete!"
