...
read -p "WARNING: This will overwrite staging database with local database. Are you sure you want to sync only the database from local to stage? (y/N) " confirm
if [[ ! "$confirm" =~ ^[yY]$ ]]; then
  echo "Sync cancelled."
  exit 1
fi
...
# Sync database
echo "Transferring and importing local database to stage..."
...
echo "Sync database from local to stage complete!"
